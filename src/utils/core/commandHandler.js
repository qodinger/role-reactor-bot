...
class CommandHandler {
  ...
  cleanup() {
    ...
    // Clean up stale command stats (entries not used in 1 hour)
    const statsCutoff = now - 60 * 60 * 1000;
    for (const [name, stats] of this.commandStats) {
      if (stats.lastUsed < statsCutoff) {
        this.commandStats.delete(name);
      } else if (stats.users.size > 1000) {
        // Trim oversized user sets to top 500 most recent
        const recentUsers = Array.from(stats.users).slice(-500);
        stats.users = new Set(recentUsers);
      }
    }
  }
