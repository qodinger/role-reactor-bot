...
class CommandHandler {
  ...
  async _executeCustomCommand(interaction) {
    ...
    const replaceVariables = async (text, member = null) => {
      ...
      // Get option values for variable replacement
      const optionsData = {};
    ...
    // Build components (buttons and select menus)
    ...
    // Get the appropriate response text (random or single)
    ...
    if (command.type === "text") {
      ...
      // Check if we should DM a specific user from options
      if (command.dmTarget) {
