...
- ✅ Rich metadata (category, requiresGuild, triggersReQuery, etc.)
...
- ✅ Validation (options validation, guild requirement)
...
import {
  ...
  getAllowedActions, // Get all available actions
...
### Example 3: Adding a Command Execution Action

```javascript
// In actionRegistry.js
execute_custom_command: {
  type: "execute_custom_command",
  category: ACTION_CATEGORIES.COMMAND_EXEC,
  requiresGuild: true,
  triggersReQuery: false,
  requiresOptions: true,
  requiredOptions: ["command"],
  description: "Execute a custom bot command",
},
```
...
4. **Action Validation**: Uses `validateActionOptions()` from registry for comprehensive validation
