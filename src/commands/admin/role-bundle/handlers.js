  ...
  try {
    ...
    if (!interaction.member.permissions.has(PermissionsBitField.Flags.ManageGuild)) {
      return interaction.editReply({
        embeds: [createErrorEmbed('Permission Denied', 'You need **Manage Server** permission to create role bundles.')]
      });
    }
    ...
    if (exists) {
      return interaction.editReply({
        embeds: [createErrorEmbed('Bundle Exists', `A bundle named **${name}** already exists. Please use a different name or delete the existing bundle first.`)]
      });
    }
    ...
    if (roles.length === 0) {
      return interaction.editReply({
        embeds: [createErrorEmbed('No Valid Roles', 'No valid roles were found. Please mention roles using @RoleName format.')]
      });
    }
    ...
    return interaction.editReply({
      embeds: [createErrorEmbed('Error', 'Failed to create role bundle. Please try again.')]
    });
  }
}
...
export async function handleDelete(interaction) {
  try {
    ...
    if (!interaction.member.permissions.has(PermissionsBitField.Flags.ManageGuild)) {
      return interaction.editReply({
        embeds: [createErrorEmbed('Permission Denied', 'You need **Manage Server** permission to delete role bundles.')]
      });
    }
    ...
    if (!exists) {
      return interaction.editReply({
        embeds: [createErrorEmbed('Not Found', `Bundle **${name}** not found.`)]
      });
    }

    // Delete bundle
    const result = await roleBundleManager.deleteByName(interaction.guild.id, name);

    if (!result.success) {
      return interaction.editReply({
        embeds: [createErrorEmbed('Error', result.error || 'Failed to delete bundle.')]
      });
    }
    ...
    return interaction.editReply({
      embeds: [createErrorEmbed('Error', 'Failed to delete role bundle. Please try again.')]
    });
  }
}
...
export async function handleList(interaction) {
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .setColor(0x5865F2)
      .setDescription(`There are **${bundles.length}** role bundle(s) in this server.`);
    ...
    return interaction.editReply({
      embeds: [createErrorEmbed('Error', 'Failed to list role bundles. Please try again.')]
    });
  }
}
