...
export async function apiPaymentStats(req, res) {
  ...
  try {
    ...
    const stats = await dbManager.payments.getGlobalStats({ startDate, endDate });
    ...
    res.json(createSuccessResponse({
      ...
      recentPayments: recentPayments.map(p => ({
        paymentId: p.paymentId, discordId: p.discordId, provider: p.provider,
        amount: p.amount, coresGranted: p.coresGranted, createdAt: p.createdAt,
      })),
      dateRange: { start: startDate, end: endDate },
    }));
  } catch (error) {
    logger.error("❌ Error getting payment stats:", error);
    const { statusCode, response } = createErrorResponse("Failed to retrieve payment statistics", 500, error.message);
...
export async function apiPendingPayments(req, res) {
  ...
  try {
    ...
    if (!dbManager?.payments) {
      ...
      return res.json(createSuccessResponse({ pending: [...pendingPaypal, ...pendingCrypto], total: pendingPaypal.length + pendingCrypto.length, source: "legacy" }));
    }
    ...
    res.json(createSuccessResponse({
      pending: pendingPayments.map(p => ({
        paymentId: p.paymentId, provider: p.provider, amount: p.amount,
        currency: p.currency, email: p.email, status: p.status, createdAt: p.createdAt,
      })),
      awaitingUserLink: paymentsWithoutUser.map(p => ({
        paymentId: p.paymentId, provider: p.provider, amount: p.amount,
        email: p.email, createdAt: p.createdAt,
      })),
      totals: { pending: pendingPayments.length, awaitingLink: paymentsWithoutUser.length },
    }));
  } catch (error) {
    logger.error("❌ Error getting pending payments:", error);
    const { statusCode, response } = createErrorResponse("Failed to retrieve pending payments", 500, error.message);
    res.status(statusCode).json(response);
  }
}
...
export async function apiGetAdminActionLogs(req, res) {
  ...
  try {
    ...
    const logs = await dbManager.payments.collection.find(query).sort({ createdAt: -1 }).limit(limit).skip(skip).toArray();
    const total = await dbManager.payments.collection.countDocuments(query);

    res.json(createSuccessResponse({
      logs: logs.map(l => ({
        logId: l.paymentId, userId: l.discordId, type: "adjustment",
        change: l.coresGranted, adminUser: l.metadata?.adminUser || "unknown",
        reason: l.metadata?.reason || "No reason provided", action: l.metadata?.action,
        originalAmount: l.metadata?.originalAmount, oldBalance: l.metadata?.previousBalance,
        newBalance: l.metadata?.newBalance, timestamp: l.createdAt,
      })),
      pagination: { total, limit, skip, hasMore: skip + logs.length < total },
    }));
  } catch (error) {
    logger.error("❌ Error getting admin action logs:", error);
    const { statusCode, response } = createErrorResponse("Failed to retrieve admin logs", 500, error.message);
    res.status(statusCode).json(response);
  }
}
