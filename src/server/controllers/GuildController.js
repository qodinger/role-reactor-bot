...
export async function apiCheckGuilds(req, res) {
  ...
  if (!guildIds || !Array.isArray(guildIds)) {
    return res.status(400).json(createErrorResponse("Guild IDs are required", 400, "Provide guildIds as an array").response);
  }

  const client = getDiscordClient();
  if (!client) return res.status(503).json(createErrorResponse("Bot client not available", 503).response);
  ...
  try {
    ...
    res.status(500).json(createErrorResponse("Failed to check guilds", 500, error.message).response);
  }
}
...
export async function apiListGuilds(req, res) {
  ...
  try {
    const guilds = await Promise.all(client.guilds.cache.map(async guild => {
      try {
        await client.guilds.fetch({ guild: guild.id, withCounts: true }).catch(() => null);
        return {
          id: guild.id,
          name: guild.name,
        ...
        return { id: guild.id, name: guild.name, icon: guild.iconURL({ dynamic: true, size: 128 }), memberCount: guild.memberCount, error: err.message };
      }
    }));
    ...
    res.status(500).json(createErrorResponse("Failed to list guilds", 500, error.message).response);
  }
}
...
export async function apiGetGuildSettings(req, res) {
  ...
  if (!guildId) return res.status(400).json(createErrorResponse("Guild ID is required", 400).response);
  ...
  try {
    ...
    if (!dbManager?.guildSettings) return res.status(503).json(createErrorResponse("GuildSettingsRepository not available", 503).response);
    ...
    const commandDetails = commandHandler.getAllCommands().map(name => {
    ...
    const isProActive = await premiumManager.isFeatureActive(guildId, PremiumFeatures.PRO.id);
    const guildStats = await GuildHelper.getEnrichedGuildStats(guildId);
    const proSubscription = await premiumManager.getSubscriptionStatus(guildId, PremiumFeatures.PRO.id);
    ...
    res.json(createSuccessResponse({
      ...
      subscription: proSubscription ? {
        ...
        lastDeductionDate: proSubscription.lastDeductionDate || proSubscription.activatedAt,
      } : null,
    ...
    res.status(500).json(createErrorResponse("Failed to retrieve guild settings", 500, error.message).response);
  }
}
...
export async function apiGetGuildChannels(req, res) {
  ...
  if (!guild) return res.status(404).json(createErrorResponse("Guild not found", 404).response);

  try {
    await guild.channels.fetch();
    const channels = guild.channels.cache
      .filter(c => c.type === 0 || c.type === 5)
      .map(c => ({ id: c.id, name: c.name, type: c.type, parentId: c.parentId, position: c.rawPosition }))
      .sort((a, b) => a.position - b.position);

    res.json(createSuccessResponse({ channels }));
  } catch (error) {
    logger.error(`❌ Error getting channels for guild ${guildId}:`, error);
    res.status(500).json(createErrorResponse("Failed to retrieve guild channels", 500, error.message).response);
  }
}
...
export async function apiGetGuildRoles(req, res) {
  ...
  try {
    ...
    const roles = guild.roles.cache.map(r => ({
      id: r.id, name: r.name, color: r.color, rawPosition: r.rawPosition,
      hoist: r.hoist, managed: r.managed, mentionable: r.mentionable,
    ...
    res.status(500).json(createErrorResponse("Failed to retrieve guild roles", 500, error.message).response);
  }
}
...
export async function apiGetGuildEmojis(req, res) {
  ...
  try {
    ...
    res.status(500).json(createErrorResponse("Failed to retrieve guild emojis", 500, error.message).response);
  }
}
...
export async function apiUpdateGuildSettings(req, res) {
  ...
  try {
    ...
    const newSettings = { ...existingSettings, ...updates, updatedAt: new Date() };

    if (updates.disabledCommands) {
      const { getPremiumManager } = await import("../../features/premium/PremiumManager.js");
      const { PremiumFeatures } = await import("../../features/premium/config.js");
      const premiumManager = getPremiumManager();
      const isActive = await premiumManager.isFeatureActive(guildId, PremiumFeatures.PRO.id);

      if (!isActive) return res.status(403).json(createErrorResponse("Pro Engine is a premium feature.", 403, "PREMIUM_REQUIRED_PRO").response);
      
      const commandHandler = getCommandHandler();
      commandHandler.syncGuildCommands(guildId, newSettings.disabledCommands).catch(err => 
        logger.error(`Failed to sync command visibility for guild ${guildId}:`, err)
      );
    }

    await dbManager.guildSettings.set(guildId, newSettings);
    res.json(createSuccessResponse({ message: "Settings updated successfully", settings: newSettings }));
  } catch (error) {
    logger.error(`❌ Error updating settings for guild ${guildId}:`, error);
    res.status(500).json(createErrorResponse("Failed to update guild settings", 500, error.message).response);
  }
}
