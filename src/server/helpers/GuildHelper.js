...
export class GuildHelper {
  ...
  static async getEnrichedGuildStats(guildId) {
    ...
    try {
      ...
      try {
        await client.guilds.fetch({ guild: guildId, withCounts: true, force: true }).catch(() => null);
      ...
      const growthHistory = await (async () => {
        try {
          const { getAnalyticsManager } = await import("../../features/analytics/AnalyticsManager.js");
      ...
      const leaderboard = await (async () => {
        try {
          ...
          const guildUsers = await storageManager.getUserExperienceLeaderboard(guild.id, 3);
      ...
      const activity = await (async () => {
        try {
          const { getStorageManager } = await import("../../utils/storage/storageManager.js");
          const storageManager = await getStorageManager();
          const guildUsers = await storageManager.getUserExperienceByGuild(guild.id);
          return {
            totalMessages: guildUsers.reduce((sum, u) => sum + (u.messagesSent || 0), 0),
            totalVoiceMins: guildUsers.reduce((sum, u) => sum + (u.voiceTime || 0), 0),
            totalCommands: guildUsers.reduce((sum, u) => sum + (u.commandsUsed || 0), 0),
            topChatters: [...guildUsers].sort((a, b) => (b.messagesSent || 0) - (a.messagesSent || 0)).slice(0, 3).map(u => ({
              username: guild.members.cache.get(u.userId)?.user.username || "Unknown",
              count: u.messagesSent || 0
            })),
            topVoiceUsers: [...guildUsers].sort((a, b) => (b.voiceTime || 0) - (a.voiceTime || 0)).slice(0, 3).map(u => ({
              username: guild.members.cache.get(u.userId)?.user.username || "Unknown",
              count: u.voiceTime || 0
            }))
          };
      ...
      return {
        ...
        onlineCount: (function() {
          const precise = guild.members.cache.filter(m => !m.user.bot && m.presence?.status && m.presence.status !== "offline").size;
          return precise || Math.max(0, (guild.approximatePresenceCount || 0) - botsInCache) || 0;
        })(),
        channelCount: guild.channels.cache.size,
        roleCount: guild.roles.cache.filter(r => !r.managed && r.id !== guild.id).size,
        ...
        growth: {
          new24h: guild.members.cache.filter(m => !m.user.bot && m.joinedTimestamp > Date.now() - 24 * 60 * 60 * 1000).size,
          new7d: guild.members.cache.filter(m => !m.user.bot && m.joinedTimestamp > Date.now() - 7 * 24 * 60 * 60 * 1000).size,
        },
        growthHistory,
        leaderboard,
        activity,
        recentMembers: (function() {
          const members = Array.from(guild.members.cache.values()).filter(m => m.user && !m.user.bot);
          return members.sort((a, b) => (b.joinedTimestamp || 0) - (a.joinedTimestamp || 0)).slice(0, 3).map(m => ({
            userId: m.user.id,
      ...
      logger.error(`Failed to fetch live guild stats for ${guildId}: ${err.message}`);
      return null;
    }
  }
  ...
  static async enrichLeaderboard(leaderboard, client) {
    ...
    for (const entry of leaderboard) {
      let userDetails = { username: "Unknown User", discriminator: "0000", avatar: null, bot: false };
      
      if (client) {
        let user = client.users.cache.get(entry.userId) || await client.users.fetch(entry.userId).catch(() => null);
        if (user) {
          userDetails = {
  ...
  static async enrichRoleMapping(messageId, mapping, guild) {
    ...
    if (rawRoles.roles && typeof rawRoles.roles === 'object' && !rawRoles.roleId) {
    ...
    for (const [emoji, roleConfig] of Object.entries(rawRoles)) {
      ...
      let roleId = typeof roleConfig === 'string' ? roleConfig : roleConfig.roleId;
      let roleName = typeof roleConfig === 'object' ? (roleConfig.roleName || roleConfig.name || "Unknown Role") : "Unknown Role";
      let roleColor = 0;
