...
describe("AI System Integration", () => {
  ...
  describe("Conversation Management", () => {
    it("should create conversation keys correctly", () => {
      const guildKey = conversationManager.getConversationKey(
        "user123",
        "guild456",
      );
    ...
    it("should parse conversation keys correctly", () => {
      const guildParsed =
        conversationManager.parseConversationKey("user123_guild456");
    ...
    it("should manage conversation history", async () => {
      ...
      // Initially empty
      let history = await conversationManager.getConversationHistory(
        userId,
        guildId,
      );
      ...
      // Check history
      history = await conversationManager.getConversationHistory(
        userId,
        guildId,
      );
    ...
    it("should separate guild and DM conversations", async () => {
      ...
      // Check separation
      const guildHistory = await conversationManager.getConversationHistory(
        userId,
        guildId,
      );
    ...
    it("should persist conversations to storage", async () => {
      ...
      expect(storedData["user123_guild456"].messages[0].content).toBe(
        "Persistent message",
      );
    });
  ...
  describe("Configuration Management", () => {
    ...
    it("should handle partial configuration updates", () => {
      ...
      expect(conversationManager.maxConversations).toBe(
        originalMaxConversations,
      );
    });
  });
  ...
  describe("Error Handling", () => {
    ...
    it("should handle storage errors gracefully", async () => {
      ...
      const history = await newManager.getConversationHistory(
        "user123",
        "guild456",
      );
      expect(history).toEqual([]);
      ...
      await expect(
        newManager.addToHistory("user123", "guild456", {
          role: "user",
